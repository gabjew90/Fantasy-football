...
def test_bye_zeroes_a_starter():
    ...
    # r1 (15) drops out, r3 (10) slides from bench into the lineup: -5
    assert full - on_bye == pytest.approx(5.0)
...
def test_simulate_monotone_and_locked():
    ...
    odds = simulate_season(strengths, matchups, records, playoff_teams=1,
                           sims=300, sigma=20.0, rng=rng,
                           points_for={1: 1400.0, 2: 1100.0, 3: 900.0})
    assert odds[1] > 0.99
    assert odds[1] > odds[2] > odds[3]
