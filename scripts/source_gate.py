...
def run_outcome(league: str, rows: pl.DataFrame, candidate: str = EXTERNAL_ARM,
    ...
    alias keys); each pair record names its candidate and rivals."""
    arms = [candidate, *rivals]
    ...
    for pair in sorted(rows["pair"].unique().to_list()):
        ...
        for slot in range(1, teams + 1):
            ...
            print(f"  {league} {pair} slot {slot:>2}: {first} {row[first]:.0f}  {candidate} {row[candidate]:.0f}"
                  f"  delta {row[candidate] - row[first]:+.0f}", flush=True)
        out.append(rec)
    return out
